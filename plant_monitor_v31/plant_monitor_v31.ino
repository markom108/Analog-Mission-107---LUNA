...
// ============================================================
//  ZDJĘCIE — jedno zdjęcie, bez pętli V
...
bool takeAndSavePhoto(uint32_t series, uint32_t trial,
  ...
  // Przepłucz pierwszą klatkę
  camera_fb_t* fb0 = esp_camera_fb_get();
  if (fb0) { esp_camera_fb_return(fb0); delay(50); }
...
void setup() {
  ...
  for (uint32_t i = 1; i <= PHOTO_COUNT; i++) {
    ...
    // Jeśli nie zapisano — tylko log błędu (już zapisany w takeAndSavePhoto)
    // Nie zapisujemy błędu do CSV — żeby nie zaśmiecać danych
